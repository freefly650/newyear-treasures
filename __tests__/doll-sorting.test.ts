...
function ids(dolls: Doll[]) {
  return dolls.map((d) => d.id);
}
...
describe("doll sorting", () => {
  it("sorts by time added newest first by default", () => {
    ...
    expect(sortDolls(dolls, "time_desc")).toHaveLength(2);
    expect(ids(sortDolls(dolls, "time_desc"))).toEqual(["2", "1"]);
  });
  ...
  it("sorts by year oldest first and pushes missing years to the bottom", () => {
    ...
    expect(ids(sortDolls(dolls, "year_asc"))).toEqual(["c", "a", "b"]);
  });
  ...
  it("sorts by line A-Z and orders year within the same line oldest->newest", () => {
    ...
    expect(ids(sortDolls(dolls, "line_asc"))).toEqual(["p2", "p1", "p3", "p4"]);
  });
  ...
  it("pushes missing line values to the bottom for line sorting", () => {
    ...
    expect(ids(sortDolls(dolls, "line_asc"))).toEqual(["withLine", "noLine"]);
  });
  ...
  it("sorts by title A-Z", () => {
    ...
    expect(ids(sortDolls(dolls, key))).toEqual(["t2", "t1"]);
  });
});
